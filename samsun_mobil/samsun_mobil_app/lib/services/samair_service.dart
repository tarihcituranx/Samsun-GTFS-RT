...
class SamAirService {
  ...
  static Future<List<Map<String, dynamic>>> getLiveSamAirBuses() async {
    ...
    try {
      // H1, H2, H3, H4 için API'ye paralel çağrı yap
      final futures = SAMAIR_LINES.map((lineCode) => http.get(Uri.parse('$ASIS_BASE/RealTimeData?lineCode=$lineCode'), headers: headers));
          ...
          try {
            ...
            for (var item in data) {
              if (item is Map<String, dynamic> && item.containsKey('Latitude')) {
                 String lCode = item['LineCode']?.toString() ?? 'SAMAIR';
                 allVehicles.add({
                    'lineCode': lCode,
