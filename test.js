...
function renderLi(h) {
    ...
    return `<div class="it pulse-it ${h.kat || 'otobus'}" onclick="shL('${encodeURIComponent(h.code)}')">
        <div class="it-badge" style="background:${ki.c}">${codeStr}</div>
        <div class="it-info">
            <div class="it-title">${nameWithoutCode}</div>
            <div class="it-sub" style="color:${subColor}"><span class="badge-icon-s" style="color:${ki.c}">${ki.i}</span> ${ki.n} &bull; ${codeStr} Hattı</div>
        </div>
...
const busIcon = (c, p) => {
    let shortPlaka = p;
    // T1 gibi hat kodlarını ayıklayalım veya doğrudan plakayı kısaltalım
    ...
    return L.divIcon({
        ...
        html: `<div class="pulse-marker" style="background-color:${c};">
                 <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><rect x="2" y="4" width="20" height="14" rx="2"/><path d="M2 9h20"/><circle cx="7" cy="19" r="1.5"/><circle cx="17" cy="19" r="1.5"/><path d="M2 13h1M21 13h1"/></svg>
                 <span class="pulse-text">${shortPlaka}</span>
               </div><div class="pulse-ring" style="border-color:${c}"></div>`,
        iconSize: [48, 28],
        iconAnchor: [24, 14]
    });
};
...
function shH() {
    ...
    Object.entries(K).forEach(([k, v]) => { const cnt = k === 'dil' ? H.length : (bk[k] ? bk[k].length : 0); x += `<div class="kb ${sK === k ? 'on' : ''}" onclick="selK('${k}')"><div class="i">${v.i}</div>${v.n}</div>` });
