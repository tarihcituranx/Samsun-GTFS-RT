...
class Http:
    ...
    def ybs_token(self):
        # YBS artık token gerektirmediği için boş string dönüyoruz
        return ""

    def ybs(self, method, submethod=None, **kw):
        # YBS sistemi tamamen devre dışı bırakılmıştır.
        log.warning(f"YBS sistemi çağrılmaya çalışıldı (pasif): {method}/{submethod}")
        return []
    ...
    def akilli_rota(self, lat1, lon1, lat2, lon2):
        ...
        try:
            ...
            for r in direct_res or []:
                ...
                try:
                    fiyat_yazi = self._get_route_price(r['code'])
                    fiyat_hesap = 0
                    if fiyat_yazi:
                        f_match = re.search(r'([\d.,]+)', str(fiyat_yazi).replace(',','.'))
                        if f_match: fiyat_hesap = float(f_match.group(1))
                except:
                    fiyat_hesap = 24.0
                    fiyat_yazi = "24.0"
                ...
                all_routes.append({
                    ...
                    'label': "Direkt",
                    'duration': yolculuk_sure + bekleme_sure,
                    'fare': fiyat_hesap,
                    'transfers': 0,
                    'co2': round((durak_sayisi * 0.1), 1),
                    'departure': kalkis_saati_str,
                    'arrival': varis_saati_str,
                    'steps': [
                        {'type': 'walk', 'duration': yurume_sure, 'label': 'Yürü', 'color': ''},
                        {'type': 'bus' if r['kat'] != 'tramvay' else 'tram', 'duration': yolculuk_sure, 'label': r['code'], 'color': '#f97316' if r['kat'] != 'tramvay' else '#22c55e', 'start_stop': r['s_ad'], 'end_stop': r['e_ad']},
                    ],
                    'desc': f"""
                    <div class="route-card direct">
                        <div class="route-header">
                            <span class="route-icon">{ "🚋" if r['kat'] == 'tramvay' else "🚌"}</span>
                            <div style="flex:1">
                        ...
                        <div style="background:#f1f8ff;padding:5px 10px;font-size:0.8rem;text-align:center;border-bottom:1px solid #ddd">
                            Tahmini Ücret: <b>{fiyat_yazi} TL</b> (Tam)
                        </div>
        ...
        try:
            ...
            for r in trans_res or []:
                ...
                try:
                    fp1 = self._get_route_price(r['hat1'])
                    f_match = re.search(r'([\d.,]+)', str(fp1).replace(',','.'))
                    fiyat_hesap = float(f_match.group(1)) if f_match else 24.0
                except:
                    fiyat_hesap = 24.0
                ...
                all_routes.append({
                    ...
                    'fare': fiyat_hesap,
                    'transfers': 1,
                    'co2': round(( (r['durak1']+r['durak2']) * 0.1), 1),
                    'departure': kalkis1_str,
                    'arrival': varis2_str,
                    'steps': [
                        {'type': 'walk', 'duration': 4, 'label': 'Yürü', 'color': ''},
                        {'type': 'bus' if r['kat1'] != 'tramvay' else 'tram', 'duration': r['durak1']*2, 'label': r['hat1'], 'color': '#f97316' if r['kat1'] != 'tramvay' else '#22c55e', 'start_stop': r['start_stop'], 'end_stop': r['transfer_name']},
                        {'type': 'walk', 'duration': 5, 'label': 'Aktarma', 'color': ''},
                        {'type': 'bus' if r['kat2'] != 'tramvay' else 'tram', 'duration': r['durak2']*2, 'label': r['hat2'], 'color': '#0ea5e9' if r['kat2'] != 'tramvay' else '#22c55e', 'start_stop': r['transfer_name'], 'end_stop': r['end_stop']},
                    ],
                    'desc': f"""
...
<head>
    ...
    <meta name="description"
        content="Samsun toplu taşıma araçlarını canlı takip edin. Otobüs, tramvay, tekne, teleferik hatları, sefer saatleri ve durak bilgileri.">
    ...
    <meta property="og:image"
        content="https://pub-bb2e103a32db4e198524a2e9ed8f35b4.r2.dev/b33a06ef-3bbb-4075-86ee-3e764c7781a9/id-preview-416cc31b--667074a0-f4e7-4cb4-9c0c-dfe85c31cdd8.lovable.app-1772983126123.png">
    ...
    <style>
        ...
        .pnl {
            ...
            transition: transform 0.3s cubic-bezier(0.4, 0, 0.2, 1), opacity 0.3s, max-height 0.3s;
        }
        ...
        .pnl-body::-webkit-scrollbar {
            width: 6px;
        }
        
        .pnl-body::-webkit-scrollbar-track {
            background: transparent;
        }
        
        .pnl-body::-webkit-scrollbar-thumb {
            background: var(--border);
            border-radius: 3px;
        }
        
        .pnl-body::-webkit-scrollbar-thumb:hover {
        ...
        .pulse-ring {
            ...
            border-radius: 24px;
            border: 3px solid;
            animation: pulsate 2s infinite cubic-bezier(0.0, 0, 0.2, 1);
            opacity: 0;
            pointer-events: none;
        }
        ...
        /* Override inline dark texts from API content in popups */
        .leaflet-popup-content [style*="color:#666"],
        .leaflet-popup-content [style*="color:#888"],
        .leaflet-popup-content [style*="color:gray"] {
...
<body class="panel-open">
    ...
    <div class="map-ctrls">
        <button class="map-ctrl" onclick="findUserLoc()" title="Konumumu Bul">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
                <path fill-rule="evenodd" d="M11.54 22.351l.07.04.028.016a.76.76 0 00.723 0l.028-.015.071-.041a16.975 16.975 0 001.144-.742 19.58 19.58 0 002.683-2.282c1.944-1.99 3.963-4.98 3.963-8.827a8.25 8.25 0 00-16.5 0c0 3.846 2.02 6.837 3.963 8.827a19.58 19.58 0 002.682 2.282 16.975 16.975 0 001.145.742zM12 13.5a3 3 0 100-6 3 3 0 000 6z" clip-rule="evenodd" />
            </svg>
        </button>
        <button class="map-ctrl" onclick="focusSamsun()" title="Samsun Merkez">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
                <path d="M11.47 3.82a.75.75 0 011.06 0l8.69 8.69a.75.75 0 11-1.06 1.06l-.968-.968v8a.75.75 0 01-.75.75h-3.75A.75.75 0 0114 20.625V15a.75.75 0 00-.75-.75h-2.5a.75.75 0 00-.75.75v5.625c0 .414-.336.75-.75.75H5.25a.75.75 0 01-.75-.75v-8L3.53 13.57a.75.75 0 11-1.06-1.06l8.69-8.69z" />
            </svg>
        </button>
    </div>
    ...
    <div class="top-bar">
        <div
            ...
            <div style="display:flex;flex-direction:column;">
                ...
                <span style="font-size:8px;color:#cbd5e1;opacity:0.8;background:rgba(255,255,255,0.15);padding:1px 5px;border-radius:4px;cursor:pointer;font-weight:700" onclick="showAboutModal()">by tarihcituranx</span>
                </div>
            </div>
        </div>
        <div style="display:flex;align-items:center;gap:10px;pointer-events:auto;">
            <div id="weatherWidget"
                style="background:rgba(15,23,42,0.6);color:#fff;padding:8px 14px;border-radius:20px;font-size:13px;font-weight:800;display:flex;align-items:center;backdrop-filter:blur(12px);border:1px solid rgba(255,255,255,0.1)">
                ⏳</div>
            <div
                style="background:rgba(16,185,129,0.2);border:1px solid #10b981;color:#10b981;font-weight:800;font-size:12px;padding:8px 14px;border-radius:20px;display:flex;align-items:center;gap:8px;backdrop-filter:blur(12px)">
                <span
                    style="width:8px;height:8px;background:#10b981;border-radius:50%;display:inline-block;animation:pulsate 2s infinite"></span>
                CANLI</div>
            <button id="settingsBtn" onclick="toggleSettings()"
                style="width:40px;height:40px;border-radius:50%;background:rgba(15,23,42,0.6);border:1px solid rgba(255,255,255,0.1);color:#fff;display:flex;align-items:center;justify-content:center;cursor:pointer;backdrop-filter:blur(12px);font-size:16px" title="Ayarlar">⚙️</button>
            <button id="themeToggle" onclick="toggleTheme()"
                style="width:40px;height:40px;border-radius:50%;background:rgba(15,23,42,0.6);border:1px solid rgba(255,255,255,0.1);color:#fff;display:flex;align-items:center;justify-content:center;cursor:pointer;backdrop-filter:blur(12px);font-size:16px">🌙</button>
        </div>
        <div id="settingsPanel" style="display:none;position:absolute;top:80px;right:16px;background:var(--bg-panel);border:1px solid var(--border);border-radius:16px;padding:16px;box-shadow:0 10px 30px rgba(0,0,0,0.2);z-index:100;width:260px;font-size:.8rem;backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);color:var(--text);pointer-events:auto">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
                <div style="font-weight:800;font-size:.9rem;display:flex;align-items:center;gap:6px">⚙️ Ayarlar</div>
                <button onclick="closeSettings()" style="background:none;border:none;font-size:1.1rem;cursor:pointer;color:var(--text)">✕</button>
            </div>
            <label style="display:flex;align-items:center;gap:10px;cursor:pointer;padding:8px 0;border-bottom:1px solid var(--border)">
                <input type="checkbox" id="chkHasilat" onchange="toggleHasilat(this)" style="width:16px;height:16px;accent-color:var(--accent)">
                <span>💰 Hasılat Değeri</span>
            </label>
            <label style="display:flex;align-items:center;gap:10px;cursor:pointer;padding:8px 0;border-bottom:1px solid var(--border)">
                <input type="checkbox" id="chkLabels" onchange="saveSetting('showLabels',this.checked)" style="width:16px;height:16px;accent-color:var(--accent)" checked>
                <span>🏷️ Durak İsimleri</span>
            </label>
            <label style="display:flex;align-items:center;gap:10px;cursor:pointer;padding:8px 0;border-bottom:1px solid var(--border)">
                <input type="checkbox" id="chkRoute" onchange="saveSetting('showRoute',this.checked)" style="width:16px;height:16px;accent-color:var(--accent)" checked>
                <span>🗺️ Güzergah Çizgisi</span>
            </label>
            <label style="display:flex;align-items:center;gap:10px;cursor:pointer;padding:8px 0;border-bottom:1px solid var(--border)">
                <input type="checkbox" id="chkAutoRefresh" onchange="saveSetting('autoRefresh',this.checked)" style="width:16px;height:16px;accent-color:var(--accent)" checked>
                <span>🔄 Otomatik Yenileme (5sn)</span>
            </label>
            <label style="display:flex;align-items:center;gap:10px;cursor:pointer;padding:8px 0;border-bottom:1px solid var(--border)">
                <input type="checkbox" id="chkAllStops" onchange="saveSetting('showAllStops',this.checked);toggleAllStops(this.checked)" style="width:16px;height:16px;accent-color:var(--accent)">
                <span>📍 Tüm Durakları Göster</span>
            </label>
            <button onclick="resetSettings()" style="margin-top:12px;width:100%;padding:10px;background:var(--bg-hover);border:1px solid var(--border);border-radius:10px;cursor:pointer;font-size:.75rem;font-weight:700;color:var(--text);font-family:inherit;transition:background 0.2s" onmouseover="this.style.background='var(--border)'" onmouseout="this.style.background='var(--bg-hover)'">🔄 Varsayılana Çevir</button>
        </div>
    </div>
    ...
    <div class="bottom-nav">
        ...
        <div class="tab" data-t="yakin">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round">
                <path d="M2 9h20M7 17.5v1M17 17.5v1M2 13h1M21 13h1M7 9v5M12 9v5M17 9v5" />
    ...
    <div id="aktarmaModal" class="modal-overlay">
        <div class="modal-content">
            <h3
                style="margin-bottom:20px;color:var(--text);font-weight:800;font-size:22px;display:flex;align-items:center;gap:10px">
                🔄 Aktarma Kuralları</h3>
            <div style="font-size:15px;color:var(--text);line-height:1.6;">
                <div
                    style="padding:16px;background:var(--bg-hover);border-radius:16px;margin-bottom:16px;border:1px solid var(--border)">
                ...
                <button onclick="document.getElementById('aktarmaModal').style.display='none'" class="bk"
                    style="margin:0">Kapat</button>
            </div>
        </div>
    </div>
    ...
    <div id="infoModal" class="modal-overlay">
        <div class="modal-content" style="text-align:center;">
            <h3 style="color:var(--accent);margin-bottom:16px;font-weight:800;font-size:20px">⚠️ Önemli Bilgilendirme
            </h3>
            <p style="font-size:15px;color:var(--text);margin-bottom:12px;font-weight:600">Bu uygulama Turan KAYA
                tarafından geliştirilen, bağımsız bir projedir.</p>
            <p style="font-size:13px;color:var(--text-muted);margin-bottom:24px;font-weight:500">Gösterilen fiyatlar,
                sefer saatleri ve araç konumları tahmini veya gecikmiş olabilir.</p>
            <div style="display:flex;align-items:center;justify-content:space-between;margin-top:16px;">
                <label
                    style="font-size:13px;color:var(--text-muted);display:flex;align-items:center;gap:8px;cursor:pointer;font-weight:600"><input
                        type="checkbox" id="chkGosterme" style="width:16px;height:16px;accent-color:var(--accent)"
                        onchange="if(this.checked) localStorage.setItem('hideInfoModal','true'); else localStorage.removeItem('hideInfoModal')">
                    Bir daha gösterme</label>
                <button onclick="closeInfoModal()" class="bk"
                    style="width:auto;margin:0;background:var(--accent);color:#fff;border:none">Anladım</button>
            </div>
        </div>
    </div>
    ...
    <div id="aboutModal" class="modal-overlay" style="display:none">
        <div class="modal-content" style="text-align:center;max-width:440px">
            ...
            <p style="font-size:13px;color:var(--text-muted);margin-bottom:16px;line-height:1.6;text-align:left">
                ...
                <strong style="color:var(--red)">Samsun Buyuksehir Belediyesi, SAMULAS A.S. veya herhangi bir resmi devlet kurumu ile hicbir bagi veya resmi ortakligi bulunmamaktadir.</strong><br><br>
                Uygulamadaki canli arac verileri ve sure tahminleri anlik ASIS/Samsun Ulasim altyapisindan cekilmekte olup bilgi amaclidir.
            </p>
            <div style="padding:12px;background:var(--bg-hover);border-radius:12px;margin-bottom:20px;border:1px solid var(--border)">
                <span style="font-size:13px;font-weight:700;color:var(--text)">Gelistirici &amp; Kaynak Kodlari:</span><br>
                <a href="https://github.com/tarihcituranx" target="_blank" style="color:var(--accent);text-decoration:none;font-weight:800;font-size:13px;display:inline-block;margin-top:4px">github.com/tarihcituranx</a>
            </div>
            <button onclick="document.getElementById('aboutModal').style.display='none'" class="bk"
                style="width:100%;margin:0;background:var(--accent);color:#fff;border:none">Kapat</button>
        </div>
    </div>

    <div id="cerezBanner"
        style="display:none;position:fixed;bottom:90px;left:50%;transform:translateX(-50%);width:90%;max-width:500px;z-index:9998;background:var(--bg-panel);border:1px solid var(--border);border-radius:24px;padding:20px;box-shadow:0 12px 40px rgba(0,0,0,0.4);backdrop-filter:blur(20px);flex-direction:column;gap:16px">
        <div style="font-size:14px;color:var(--text);line-height:1.6;font-weight:500">🍪 Bu uygulama yalnızca temel
            işlevsellik için <strong>localStorage</strong> kullanır.</div>
        <div style="display:flex;gap:12px;width:100%">
            <button onclick="cerezReddet()" class="bk" style="margin:0;flex:1;">Sadece Zorunlu</button>
            <button onclick="cerezKabul()" class="bk"
                style="margin:0;flex:1;background:var(--accent);color:#fff;border:none">Kabul Et</button>
        </div>
    </div>

    <script src="/static/leaflet.js"></script>
    <script>
// ===== OTOMATİK ÖNBELLEK TEMİZLEME (YENİ SÜRÜM KONTROLÜ) =====
...
(function() {
    ...
    if (savedVersion && savedVersion !== BUILD_VERSION) {
        ...
        console.log("Yeni sürüm (" + BUILD_VERSION + ") yüklendi, önbellek temizleniyor...");
        setTimeout(() => {
        ...
        function toggleFavoriteHat(id) {
            let favs = getFavorites('fav_hatlar');
            ...
            localStorage.setItem('fav_hatlar', JSON.stringify(favs));
            return idx === -1;
        }
        function toggleFavoriteDurak(durakObj) {
            let favs = getFavorites('fav_duraklar');
            const idx = favs.findIndex(d => d.id === durakObj.id || d.kod === durakObj.kod);
            if (idx > -1) {
                favs.splice(idx, 1);
            } else {
                favs.push(durakObj);
            }
            localStorage.setItem('fav_duraklar', JSON.stringify(favs));
            return idx === -1;
        }
        function isFavoriteHat(id) {
            return getFavorites('fav_hatlar').includes(id);
        }
        function isFavoriteDurak(id) {
            return getFavorites('fav_duraklar').some(d => d.id === id || d.kod === id);
        }

        // Harita konumlandırma fonksiyonları
        function focusSamsun() {
            if (window.map) {
                map.setView([41.29, 36.33], 13);
            }
        }
            ...
            if (navigator.geolocation) {
                navigator.geolocation.getCurrentPosition(position => {
                    ...
                    L.popup()
                        ...
                        .setContent("Konumunuz")
                        .openOn(map);
                }, () => {
                    showToast("Konumunuza erişilemedi.");
                });
            } else {
        ...
        function applyTheme(t) {
            ...
            const klL = document.getElementById('klL'), klD = document.getElementById('klD');
            if (klL && klD) { klL.style.display = t === 'dark' ? 'none' : 'block'; klD.style.display = t === 'dark' ? 'block' : 'none' }
        ...
        const K = {
            dil: { i: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" style="width:100%;height:100%"><circle cx="12" cy="12" r="10"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/><path d="M2 12h20"/></svg>', n: 'Tümü', c: '#333' },
            ...
            odak: { i: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" style="width:100%;height:100%;color:#16a34a"><rect x="3" y="10" width="18" height="8" rx="1.5"/><path d="M3 6c0-1.5 1-2 3-2h12c2 0 3 .5 3 2v4H3z"/><circle cx="7" cy="18" r="1.5"/><circle cx="17" cy="18" r="1.5"/><path d="M7 6h3M14 6h3"/><path d="M7 13h10"/></svg>', n: 'Odak', c: '#16a34a' },
            ...
            havalimani: { i: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" style="width:100%;height:100%;color:#dc2626"><rect x="2" y="7" width="20" height="10" rx="1.5"/><circle cx="7" cy="17" r="1.5"/><circle cx="17" cy="17" r="1.5"/><path d="M6 7v4M12 7v4M18 7v4"/><path d="M9 13.5h3.5l1.5 1.2h0.8v-0.6l-1-1.2 1-0.6H13.3l-1.5 1.2H9z" fill="currentColor"/></svg>', n: 'SamAir', c: '#dc2626' },
        ...
        function getDisplayCode(h) {
            if (!h) return '';
            const shortName = h.gtfs_route_short_name || h.short_name;
            if (shortName && shortName.trim()) return shortName.trim();
            const code = h.code || '';
            const name = h.name || '';
            if (code.length > 8) {
                const match = name.match(/\b(E\d+)\b/);
                if (match) return match[1];
                if (code.toLowerCase().includes('ekspres') || name.toLowerCase().includes('ekspres')) return 'EXP';
        ...
        function renderLi(h) {
            const ki = K[h.kat || 'otobus'] || K.otobus;
            const nameStr = h.name || h.code;
            const codeStr = h.code || '';
            const displayCode = getDisplayCode(h);
            const fontSize = displayCode.length > 5 ? '10px' : (displayCode.length > 3 ? '12px' : '15px');
            const nameWithoutCode = nameStr.startsWith(codeStr) ? nameStr.substring(codeStr.length).trim().replace(/^-/, '').trim() : nameStr;
            const isDark = document.documentElement.getAttribute('data-theme') === 'dark';
            const subColor = isDark ? '#94a3b8' : '#64748b';
            return `<div class="it pulse-it ${h.kat || 'otobus'}" onclick="shL('${encodeURIComponent(h.code)}')">
        <div class="it-badge" style="background:${ki.c};font-size:${fontSize}">${displayCode}</div>
        <div class="it-info">
            <div class="it-title">${nameWithoutCode}</div>
            <div class="it-sub" style="color:${subColor}"><span class="badge-icon-s" style="color:${ki.c}">${ki.i}</span> ${ki.n} &bull; ${displayCode} Hattı</div>
        </div>
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="it-arrow"><path d="M9 18l6-6-6-6"/></svg>
    </div>`;
        }

        const busIcon = (c, p, lineShort = '') => {
            let shortPlaka = p;
            if (p && p.length > 5 && !p.includes('T')) shortPlaka = p.substring(p.length - 4);
            else if (p) shortPlaka = p.split(' ')[0];

            const displayText = lineShort ? `${lineShort} (${shortPlaka})` : shortPlaka;
            const textLen = displayText.length;
            const width = Math.max(48, 30 + textLen * 6.5);
            const height = 28;

            return L.divIcon({
                className: 'pulse-marker-wrapper',
                html: `<div class="pulse-marker" style="background-color:${c}; width: 100%; height: 100%; justify-content: center; white-space: nowrap; padding: 4px 6px; box-sizing: border-box; display: flex; align-items: center; gap: 4px; border-radius: 14px;">
                 <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" style="flex-shrink: 0;"><rect x="2" y="4" width="20" height="14" rx="2"/><path d="M2 9h20"/><circle cx="7" cy="19" r="1.5"/><circle cx="17" cy="19" r="1.5"/><path d="M2 13h1M21 13h1"/></svg>
                 <span class="pulse-text" style="font-size: 11px; font-weight: 800; letter-spacing: -0.1px;">${displayText}</span>
               </div><div class="pulse-ring" style="border-color:${c}; border-radius: 18px;"></div>`,
                iconSize: [width, height],
                iconAnchor: [width / 2, height / 2]
            });
        };
        const bI = busIcon;
        const stopIcon = (n, name = '') => {
            if (name) return stopLbl(name, n, '#3b82f6');
            return L.divIcon({ className: '', html: `<div style="width:14px;height:14px;background:#3b82f6;border-radius:50%;border:2px solid #fff;box-shadow:0 1px 4px rgba(0,0,0,.3)"></div>`, iconSize: [14, 14], iconAnchor: [7, 7] });
        };
        const dI = (n, c) => L.divIcon({ className: '', html: `<div style="width:14px;height:14px;background:${c};border-radius:50%;border:2px solid #fff;box-shadow:0 1px 4px rgba(0,0,0,.3)"></div>`, iconSize: [14, 14], iconAnchor: [7, 7] });
...
Güncelleme: ${trTime}`;
        ...
        async function requestLocation() {
            ...
            const lastLoc = saved ? JSON.parse(saved) : defLoc;
            if (!navigator.geolocation) { userLoc = lastLoc; map.setView([lastLoc.lat, lastLoc.lon], 15); loadHats(); showToast("Tarayıcınız konum servisini desteklemiyor."); return }
            ...
            navigator.geolocation.getCurrentPosition(async p => {
                ...
                userLoc = lastLoc; map.setView([lastLoc.lat, lastLoc.lon], 15); L.marker([lastLoc.lat, lastLoc.lon]).addTo(map).bindPopup(saved ? "📍 Son Bilinen Konum" : "Samsun Meydan").openPopup(); loadHats();
                showToast((reasons[err.code] || 'Konum hatası: ' + err.message) + (saved ? " (Son konum)" : "")); console.warn('Geolocation error:', err.code, err.message);
                // Show locate button for retry (mobile needs user gesture)
                if (!document.getElementById('locateBtn') && err.code !== 1) {
            ...
            }, { enableHighAccuracy: true, timeout: 15000, maximumAge: 300000 }); // Artırılmış cache süresi (5 dk)
        }
        async function init() {
        ...
        async function shYakin(duraklar) {
            ...
            if (duraklar && duraklar.length) {
                duraklar.forEach((d, i) => {
                    ...
                    M['d' + (d.id || d.kod)] = L.marker([d.lat, d.lon], { icon: stopIcon(i + 1, d.ad) })
                        .addTo(map)
...
if (p) p.classList.remove('minimized');
            document.getElementById('ct').innerHTML = '<div class="loading">Durak bilgileri alınıyor...</div>'; try {
                const inf = await (await fetch(`/api/durak_panel/${kod}`)).json();
                let x = `<button class="bk" onclick="shYakin()">← Geri</button><div class="sec">🚏 Durak: ${kod}</div>`;
                Object.values(V).forEach(m => map.removeLayer(m));
                V = {};
                const activeBuses = [];
                if (inf.length) {
                    x += '<div style="display:flex;flex-direction:column;gap:2px">';
                    x += '<div class="sec" style="font-size:.75rem;margin:4px 0">🚌 Geçen Hatlar</div><div class="lst" style="max-height:40vh;overflow-y:auto">';
                    inf.forEach(h => {
                        const ki = K[h.kat || 'otobus'] || K.otobus;
                        const displayCode = h.hat;
                        const fontSize = displayCode.length > 5 ? '10px' : (displayCode.length > 3 ? '12px' : '15px');
                        const isDark = document.documentElement.getAttribute('data-theme') === 'dark';
                        const subColor = isDark ? '#94a3b8' : '#64748b';
                        ...
                        if (h.gelen) {
                            ...
                            if (h.gelen.verify) {
                                ...
                                if (v.status === 'OK') vb = '<span style="color:#fff;background:var(--green);padding:1px 5px;border-radius:4px;font-size:0.55rem;font-weight:700;margin-left:4px">✅ Doğrulandı</span>';
                                else if (v.status === 'WARN') vb = `<span style="color:#fff;background:#f59e0b;padding:1px 5px;border-radius:4px;font-size:0.55rem;font-weight:700;margin-left:4px">⚠️ ${v.msg}</span>`;
                                else if (v.status === 'ERR') vb = `<span style="color:#fff;background:var(--red);padding:1px 5px;border-radius:4px;font-size:0.55rem;font-weight:700;margin-left:4px">❌ ${v.msg}</span>`;
                                else vb = `<span style="color:#fff;background:var(--accent);padding:1px 5px;border-radius:4px;font-size:0.55rem;font-weight:700;margin-left:4px">ℹ️ ${v.msg}</span>`;
                            }
                            
                            if (h.gelen.lat && h.gelen.lon) {
                                const m = L.marker([h.gelen.lat, h.gelen.lon], { icon: bI(K[h.kat].c, h.gelen.plaka, h.hat) }).addTo(map).bindPopup(`<b>${h.hat}</b><br>${h.gelen.tahmini_dk} dk`);
                                V['v' + h.gelen.plaka] = m;
                                activeBuses.push([h.gelen.lat, h.gelen.lon]);
                            }
                            
                            liveHtml = `<div class="live-badge" style="margin-top:6px;padding:6px 10px;background:rgba(16,185,129,0.1);border:1px solid rgba(16,185,129,0.2);border-radius:8px;font-size:0.7rem;color:var(--text);font-weight:700">
                                <span style="color:var(--green)">⏱️ ${h.gelen.tahmini_dk} dk</span> (${h.gelen.durak_kaldi} durak) ${vb}
                                <div style="font-weight:400;font-size:0.6rem;color:var(--text-muted);margin-top:2px">Plaka: ${h.gelen.plaka} &bull; Hız: ${h.gelen.hiz} km/s &bull; Yolcu: ${h.gelen.doluluk || 0}</div>
                            </div>`;
                        }
                        
                        x += `<div class="it pulse-it ${h.kat || 'otobus'}" onclick="shL('${encodeURIComponent(h.hat)}')" style="flex-direction:column;align-items:stretch;gap:4px;padding:10px 14px;cursor:pointer;margin-bottom:8px">
                            <div style="display:flex;align-items:center;width:100%;min-width:0;gap:8px">
                                <div class="it-badge" style="background:${ki.c};font-size:${fontSize};flex-shrink:0">${displayCode}</div>
                                <div class="it-info" style="min-width:0;flex:1">
                                    <div class="it-title" style="font-weight:700;font-size:14px">${h.ad}</div>
                                    <div class="it-sub" style="color:${subColor};font-size:11px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis"><span class="badge-icon-s" style="color:${ki.c}">${ki.i}</span> ${ki.n} &bull; ${displayCode} Hattı</div>
                                </div>
                                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="it-arrow" style="flex-shrink:0;width:16px;height:16px"><path d="M9 18l6-6-6-6"/></svg>
                            </div>
                            ${liveHtml}
                        </div>`;
                    });
                    ...
                    x += '<div class="sec" style="font-size:.75rem;margin:8px 0 4px">📍 Yaklaşan Araçlar</div><div class="lst">';
                    const approaching = inf.filter(h => h.gelen);
                    if (approaching.length > 0) {
                        approaching.forEach(h => {
                            const g = h.gelen;
                            x += `<div class="arac pulse-it" onclick="map.setView([${g.lat},${g.lon}],16)" style="padding:10px 14px;cursor:pointer;margin-bottom:8px">
                                <div style="display:flex;justify-content:space-between;align-items:center;width:100%;gap:10px;min-width:0">
                                    <div style="min-width:0;flex:1">
                                        <div style="font-weight:800;font-size:.85rem;color:var(--text);display:flex;align-items:center;gap:6px">
                                            <span class="badge-icon-s" style="color:var(--accent)">🚌</span> ${h.hat}
                                        </div>
                                        <div style="font-size:.65rem;color:var(--text-muted);margin-top:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">Plaka: ${g.plaka}</div>
                                    </div>
                                    <div style="text-align:right;flex-shrink:0">
                                        <div style="font-weight:800;font-size:1rem;color:var(--green)">${g.tahmini_dk} dk</div>
                                        <div style="font-size:.6rem;color:var(--text-muted)">${g.durak_kaldi} durak &bull; ${g.hiz} km/s</div>
                                    </div>
                                </div>
                            </div>`;
                        });
                    } else {
                        x += '<div class="no-data" style="font-size:.7rem">Yaklaşan araç bulunamadı</div>';
                    }
                    x += '</div></div>';
        ...
        function attachTabEvents() {
            document.querySelectorAll('.tab').forEach(t => t.onclick = () => {
                ...
                // Handle Bottom Sheet UI Open/Close
                if (cur === 'harita' || cur === 'admin') {
                ...
                else if (cur === 'yakin') shYakin();
                else if (cur === 'odak') shO();
                else if (cur === 'kesfet') shKesfet();
                else if (cur === 'samair') shS();
                else shS();
            });
        }
        attachTabEvents();
        ...
        let poiData = [];
        async function shKesfet(filterCat = '') {
            clr(); document.getElementById('ct').innerHTML = '<div class="loading">⏳</div>'; try {
                if (!poiData.length) poiData = await (await fetch('/api/mekanlar')).json(); const cats = ['Tümü', 'Müzeler', 'Anıtlar', 'Tarihi Mekanlar']; let x = `<style>.k-logo{width:140px;height:140px;object-fit:cover;border-radius:50%;box-shadow:var(--shadow2);border:3px solid var(--card-border);transition:transform 0.3s;margin:0 auto}</style>`; x += `<div style="text-align:center;padding:16px 0 8px"><img src="/static/images/kesfet_light.jpg" class="k-logo" id="klL" style="display:${document.documentElement.getAttribute('data-theme') === 'dark' ? 'none' : 'block'}" onerror="this.style.display='none'" width="140" height="140" loading="lazy"><img src="/static/images/kesfet_dark.jpg" class="k-logo" id="klD" style="display:${document.documentElement.getAttribute('data-theme') === 'dark' ? 'block' : 'none'}" onerror="this.style.display='none'" width="140" height="140" loading="lazy"></div>`; x += `<div class="sec" style="text-align:center;font-size:1.2rem;margin-top:0">🏛️ Samsun'u Keşfet</div>`; x += '<div class="kg">'; cats.forEach(c => { const active = (!filterCat && c === 'Tümü') || (filterCat === c); x += `<div class="kb${active ? ' on' : ''}" onclick="shKesfet('${c === 'Tümü' ? '' : c}')"><span class="i">${c === 'Müzeler' ? '🏛' : c === 'Anıtlar' ? '🗽' : c === 'Tarihi Mekanlar' ? '🏰' : '🌍'}</span>${c}</div>` }); x += '</div>'; const filtered = filterCat ? poiData.filter(p => p.cat === filterCat) : poiData; x += '<div class="lst" style="max-height:50vh">'; filtered.forEach(p => { x += `<div class="drk" onclick="shMekanDetay(${p.id})" style="gap:10px;padding:10px;cursor:pointer"><img src="${p.img}" style="width:48px;height:48px;border-radius:8px;object-fit:cover;flex-shrink:0" onerror="this.style.display='none'" width="48" height="48" loading="lazy"><div class="inf"><span class="ad">${p.title}</span><span class="fyt">${p.cat} • ${p.hours}</span></div></div>` }); x += '</div>'; x += '<div style="text-align:center;margin-top:8px;font-size:.6rem;color:var(--text3)">Kaynak: <a href="https://samsunkesfet.com" target="_blank" style="color:var(--accent)">samsunkesfet.com</a></div>'; document.getElementById('ct').innerHTML = x;
                // Haritada marker'ları göster
                filtered.forEach((p, i) => { const icon = L.divIcon({ className: '', html: `<div style="position:relative"><div style="width:28px;height:28px;background:#9333ea;border:2px solid #fff;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:14px;color:#fff;box-shadow:0 2px 8px rgba(0,0,0,.3)">${p.cat === 'Müzeler' ? '🏛' : p.cat === 'Anıtlar' ? '🗽' : '🏰'}</div><div style="position:absolute;top:-4px;left:32px;white-space:nowrap;font-size:10px;font-weight:700;color:var(--text);background:var(--panel);padding:2px 6px;border-radius:4px;border:1px solid var(--card-border);pointer-events:none">${p.title}</div></div>`, iconSize: [28, 28], iconAnchor: [14, 28] }); M['poi_' + i] = L.marker([p.lat, p.lon], { icon }).addTo(map).bindPopup(`<b>${p.title}</b><br><small>${p.cat}</small><br><img src="${p.img}" style="width:120px;border-radius:6px;margin:4px 0"><br><button onclick="shMekanDetay(${p.id})" style="padding:4px 8px;font-size:.7rem;cursor:pointer;border:1px solid #ccc;border-radius:4px">Detay Gör</button>`) }); if (filtered.length) { const bounds = filtered.map(p => [p.lat, p.lon]); map.fitBounds(bounds, { padding: [40, 40] }) }
            } catch (e) { console.error(e) }
        }

        // ===== HAT LİSTESİ =====
        function normalizeStr(text) { if (!text) return ""; let t = text.toLowerCase(); const rep = { 'ı': 'i', 'ğ': 'g', 'ü': 'u', 'ş': 's', 'ö': 'o', 'ç': 'c', 'i̇': 'i', ' ': '' }; for (let k in rep) t = t.split(k).join(rep[k]); return t }

        function shH() {
            const bk = {}; H.forEach(h => { const k = h.kat || 'otobus'; (bk[k] = bk[k] || []).push(h) });
            ...
            x += `<div class="kg" id="cat-grid" style="display:flex">`;
            Object.entries(K).forEach(([k, v]) => {
                const cnt = k === 'dil' ? H.length : (bk[k] ? bk[k].length : 0);
                if (cnt > 0 || k === 'dil' || k === 'odak' || k === 'havalimani') {
                    x += `<div class="kb ${sK === k ? 'on' : ''}" onclick="selK('${k}')"><div class="i">${v.i}</div>${v.n}</div>`;
                }
            });
            x += `</div><div class="lst" id="lst">`; (sK && sK !== 'dil' ? bk[sK] || [] : H).forEach(h => { x += renderLi(h) }); document.getElementById('ct').innerHTML = x + `</div>`
        }
        window.selK = k => {
            if (k === 'odak') {
                shO();
        ...
        window.flt = q => { q = normalizeStr(q); const bk = {}; H.forEach(h => { const k = h.kat || 'otobus'; (bk[k] = bk[k] || []).push(h) }); const f = (sK && sK !== 'dil' ? bk[sK] || [] : H).filter(h => (normalizeStr(h.code + h.name)).includes(q)); document.getElementById('lst').innerHTML = f.map(h => renderLi(h)).join('') };

        // ===== ARAÇ GÜNCELLEME =====
        function saveSetting(k, v) { localStorage.setItem(k, v ? '1' : '0') }
        function getSetting(k, def) {
            if (k === 'showHasilat') {
                if (localStorage.getItem('hasilatUnlocked') !== 'true') return false;
            }
            const v = localStorage.getItem(k);
            if (v === null) return def;
            return v === '1';
        }
        async function toggleHasilat(el) {
            if (el.checked) {
                if (localStorage.getItem('hasilatUnlocked') !== 'true') {
                    const pwd = prompt("Lütfen admin şifresini giriniz:");
                    if (!pwd) {
                        el.checked = false;
                        return;
                    }
                    try {
                        const r = await fetch('/api/admin/verify?key=' + encodeURIComponent(pwd));
                        if (r.ok) {
                            localStorage.setItem('hasilatUnlocked', 'true');
                            saveSetting('showHasilat', true);
                            showToast("Hoş geldiniz, hasılat bilgisi aktif edildi.");
                            if (window.lastVArgs) {
                                upV(window.lastVArgs.e, window.lastVArgs.col, window.lastVArgs.duraklar, window.lastVArgs.lineShort);
                            }
                        } else {
                            showToast("Hatalı admin şifresi!");
                            el.checked = false;
                            saveSetting('showHasilat', false);
                        }
                    } catch (err) {
        ...
        function resetSettings() { localStorage.removeItem('showHasilat'); localStorage.removeItem('hasilatUnlocked'); localStorage.removeItem('showLabels'); localStorage.removeItem('showRoute'); localStorage.removeItem('autoRefresh'); localStorage.removeItem('showAllStops'); localStorage.removeItem('theme'); localStorage.removeItem('cerezOnay'); localStorage.removeItem('userLoc'); localStorage.removeItem('hideInfoModal'); applyTheme(window.matchMedia('(prefers-color-scheme:dark)').matches ? 'dark' : 'light'); loadSettingsUI(); toggleAllStops(false); showToast('Ayarlar varsayılana çevrildi'); if (window.lastVArgs) { upV(window.lastVArgs.e, window.lastVArgs.col, window.lastVArgs.duraklar, window.lastVArgs.lineShort); } }
        let allStopMarkers = [];
        async function toggleAllStops(show) {
            ...
            try {
                ...
                stops.forEach(s => {
                    const sIcon = L.divIcon({ className: '', html: `<div style="position:relative"><div style="width:12px;height:12px;background:#6366f1;border:2px solid #fff;border-radius:50%;box-shadow:0 1px 4px rgba(0,0,0,.4)"></div>${showLabels ? `<div style="position:absolute;top:-2px;left:16px;white-space:nowrap;font-size:10px;font-weight:600;color:var(--text);background:var(--panel);padding:1px 4px;border-radius:3px;border:1px solid var(--border);pointer-events:none;opacity:.85">${s.ad}</div>` : ''}</div>`, iconSize: [12, 12], iconAnchor: [6, 6] });
        ...
        async function upV(e, col, duraklar = [], lineShort = '') { window.lastVArgs = { e, col, duraklar, lineShort }; try { const aa = await (await fetch('/api/hat/arac/' + e)).json(); Object.values(V).forEach(m => map.removeLayer(m)); V = {}; let html = ''; const showH = getSetting('showHasilat', false); document.querySelectorAll('.drk .vtg').forEach(el => el.remove()); if (Array.isArray(aa) && aa.length > 0) { document.getElementById('acnt').innerText = aa.length; aa.forEach(a => { V['v' + a.plaka] = L.marker([a.lat, a.lon], { icon: bI(col, a.plaka, lineShort || e) }).addTo(map); const yak = a.yakin || ''; let kalanStr = ''; let cevreDuraklar = ''; if (yak && duraklar && duraklar.length) { const yi = duraklar.findIndex(d => d.ad.includes(yak) || yak.includes(d.ad)); if (yi !== -1) { const kalan = duraklar.length - (yi + 1); kalanStr = `<span style="background:var(--red);color:#fff;padding:2px 6px;border-radius:12px;font-size:0.6rem;margin-left:8px;box-shadow:0 2px 4px rgba(220,38,38,0.3)">🏁 Son durağa ${kalan} durak</span>`; let p2 = duraklar[yi - 2], p1 = duraklar[yi - 1], n1 = duraklar[yi + 1], n2 = duraklar[yi + 2]; cevreDuraklar += `<div style="font-size:0.65rem;color:var(--text);margin-top:8px;border-top:1px dashed var(--border);padding-top:8px">`; if (p2) cevreDuraklar += `<div style="color:var(--text3);margin-bottom:2px">↑ ${p2.ad}</div>`; if (p1) cevreDuraklar += `<div style="color:var(--text2);margin-bottom:2px">↑ ${p1.ad}</div>`; cevreDuraklar += `<div style="color:var(--accent);font-weight:700;margin:4px 0;background:var(--accent-bg);padding:2px 6px;border-radius:4px;display:inline-block">📍 ${yak}</div>`; if (n1) cevreDuraklar += `<div style="color:var(--text2);margin-top:2px">↓ ${n1.ad}</div>`; if (n2) cevreDuraklar += `<div style="color:var(--text3);margin-top:2px">↓ ${n2.ad}</div>`; cevreDuraklar += `</div>`; } } const durumBadge = a.durum === 'dikkat' ? `<span style="background:#f59e0b;color:#000;padding:1px 6px;border-radius:8px;font-size:0.55rem;font-weight:700;margin-left:4px;vertical-align:middle">Durdu</span>` : a.durum === 'uyari' ? `<span style="background:#f97316;color:#fff;padding:1px 6px;border-radius:8px;font-size:0.55rem;font-weight:700;margin-left:4px;vertical-align:middle">Yavaş</span>` : ''; html += `<div class="arac" onclick="map.setView([${a.lat},${a.lon}],16)" style="flex-wrap:wrap"><div style="display:flex;justify-content:space-between;width:100%;align-items:center;gap:8px"><div style="min-width:0;flex:1"><div class="pl" style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis">🔹 ${a.plaka}${kalanStr}</div></div><div style="text-align:right;flex-shrink:0"><div style="font-weight:800;font-size:.9rem;color:var(--text)">${a.hiz} <small style="font-weight:400;font-size:0.6rem">km/s</small>${durumBadge}</div><div style="font-size:.65rem;color:var(--text2);margin-top:2px">${a.saat ? '⏱ ' + a.saat : ''}</div></div></div>${cevreDuraklar || (yak ? `<div class="inf" style="color:var(--accent);font-weight:700;margin-top:6px;background:var(--accent-bg);padding:2px 6px;border-radius:4px;display:inline-block">📍 ${yak}</div>` : '')}<div style="display:flex;gap:6px;flex-wrap:wrap;width:100%;margin-top:10px;padding-top:10px;border-top:1px solid var(--border);font-size:.65rem;color:var(--text)"><span style="background:var(--bg-hover);padding:4px 8px;border-radius:12px;border:1px solid var(--border)">👥 <b>${a.yolcu}</b> biniş</span><span style="background:var(--bg-hover);padding:4px 8px;border-radius:12px;border:1px solid var(--border)">📊 Gün: <b>${a.gunluk_yolcu || 0}</b></span><span style="background:var(--bg-hover);padding:4px 8px;border-radius:12px;border:1px solid var(--border)">🏎 Max: <b>${a.max_hiz || 0}</b></span><span style="background:var(--bg-hover);padding:4px 8px;border-radius:12px;border:1px solid var(--border)">📏 <b>${a.mesafe_km || 0}</b> km</span>${showH ? `<span style="background:var(--bg-hover);padding:4px 8px;border-radius:12px;border:1px solid var(--border)">💰 <b>₺${(a.hasilat || 0).toFixed(0)}</b></span>` : ''}</div><div style="width:100%;margin-top:10px;padding-top:8px;border-top:1px dashed var(--border);font-size:0.56rem;color:var(--text-muted);display:grid;grid-template-columns:1fr 1fr;gap:4px;line-height:1.25"><div>👥 <b>Biniş:</b> Anlık sefer yolcu sayısı</div><div>📊 <b>Gün:</b> Bugün toplam biniş</div><div>🏎 <b>Max:</b> Sefer en yüksek hız</div><div>📏 <b>Mesafe:</b> Günlük toplam km</div>${showH ? `<div>💰 <b>Hasılat:</b> Günlük toplam tahmini gelir (₺)</div>` : ''}</div></div>`; if (yak) { const rows = document.querySelectorAll('.drk'); rows.forEach(r => { if (r.innerText.includes(yak)) { if (!r.querySelector('.vtg')) r.innerHTML += `<span class="vtg" style="background:${col};color:#fff;padding:2px 6px;border-radius:4px;font-size:0.6rem;margin-left:6px">🚌 ${a.plaka}</span>` } }) } }); document.getElementById('vlist').innerHTML = html } else { document.getElementById('acnt').innerText = '0'; document.getElementById('vlist').innerHTML = '<div style="text-align:center;padding:10px;color:var(--text3);font-size:0.7rem">Aktif araç yok</div>' } } catch (e) { console.error(e) } }
...
if (p) p.classList.remove('minimized');
            if (window.innerWidth <= 480) togglePnl(false); clr(); document.getElementById('ct').innerHTML = '<div class="loading" style="display:flex;flex-direction:column;align-items:center;gap:8px"><div style="font-size:2rem;animation:spin 1s linear infinite">🔄</div><div style="color:var(--text);font-size:.8rem;font-weight:600">Hat bilgileri yükleniyor...</div></div><style>@keyframes spin{from{transform:rotate(0)}to{transform:rotate(360deg)}}</style>'; try {
                const _results = await Promise.allSettled([
                    ...
                    fetch('/api/hat/sefer/' + e).then(r => r.ok ? r.json() : []),
                    fetch('/api/hat/arac/' + e).then(r => r.ok ? r.json() : []),
                    fetch('/api/hat/esles/' + e).then(r => r.ok ? r.json() : {}),
                    fetch('/api/hat/fiyat/' + e).then(r => r.ok ? r.json() : {})
                ]);
                const inf = _results[0].status === 'fulfilled' ? _results[0].value : {};
                const dr = _results[1].status === 'fulfilled' ? _results[1].value : [];
        ...
        <div style="margin:10px 0;border:1px solid var(--card-border);border-radius:10px;overflow:hidden">
            ...
            <div id="tramTabContent" style="padding:10px;background:var(--card);overflow-x:auto">
                ...
                x += `<div class="fiyat"><div class="t">Bilet</div><div class="pv">₺${tamF}</div><div class="s">İndirimli ₺${indF}${fy.aktarma1 ? ' | Aktarma: ' + fy.aktarma1 : ''}</div></div>`; x += `<div class="araclar"><div class="t">🚌 Canlı Araçlar</div><div id="vlist">Yükleniyor...</div></div>`; if (sa.length) {
                    ...
                    if (hi.length || hs.length) { x += `<div class="saat"><div class="t">📅 Saatler</div><div class="saattab"><div class="on" onclick="schT('hi',this)">Hİ (${hi.length})</div><div onclick="schT('hs',this)">HS (${hs.length})</div></div><div class="saatlar" id="scht">${hi.slice(0, 40).map(s => `<span>${s.saat}</span>`).join('')}${hi.length > 40 ? `<span>+${hi.length - 40}</span>` : ''}</div></div>`; window._s = { hi, hs } }
                } if (da.length) { x += `<div class="sec">📍 Duraklar (${da.length})</div>`; const co = []; const stopCol = col; const showLbl = getSetting('showLabels', true); da.forEach((d, i) => { x += `<div class="drk" onclick="map.setView([${d.lat},${d.lon}],17)"><span class="no" style="background:${stopCol}">${i + 1}</span><span class="inf"><span class="ad">${d.ad}</span></span></div>`; if (d.lat && d.lon) { co.push([d.lat, d.lon]); M['d' + i] = L.marker([d.lat, d.lon], { icon: showLbl ? stopLbl(d.ad, i + 1, stopCol) : dI(i + 1, stopCol) }).addTo(map) } }); if (co.length) { map.fitBounds(co, { padding: [40, 40] }); if (getSetting('showRoute', true)) { if (k === 'teleferik' || k === 'tekne') { M['routeLine'] = L.polyline(co, { color: col, weight: 4, opacity: 0.8, dashArray: '8,6' }).addTo(map) } else { drawRouteOSRM(co, col) } } } } else x += `<div class="no-data">📍 Durak bilgisi yok</div>`; document.getElementById('ct').innerHTML = x; const lineShortName = inf.short_name || inf.displayCode || e; upV(e, col, da, lineShortName); if (getSetting('autoRefresh', true)) liveT = setInterval(() => upV(e, col, da, lineShortName), 5000)
            } catch (e) { console.error(e); document.getElementById('ct').innerHTML = `<button class="bk" onclick="shH()">← Hatlar</button><div class="no-data">❌ Hata</div>` }
        }
        window.shL = shL;
        ...
        if (bdPanel) {
            ...
            bdPanel.addEventListener('touchend', () => {
                ...
                if (currentY > 100) {
                    // Swiped down: minimize panel without clearing map
                    bdPanel.classList.add('minimized');
                }
                startY = 0;
                currentY = 0;
            });

            // Tap on minimized panel bar to re-open
            bdPanel.addEventListener('click', (e) => {
                if (bdPanel.classList.contains('minimized')) {
                    bdPanel.classList.remove('minimized');
                }
            });
        }
...
def create_app(db, col):
    ...
    # ==========================================
    MEKANLAR = [
        {"id":1,"title":"Kent Müzesi","desc":"Samsun'un tarihi ve kültürel mirasını sergileyen önemli bir müze.","cat":"Müzeler","img":"https://samsunkesfet.com/media/img/muze/muzekapak/kent-muzesi-kapak.jpg","lat":41.286390,"lon":36.340000,"hours":"09:00 - 17:30","audio":{"tr":"https://samsunkesfet.com/media/sound/tr/km/101.mp3"},"code":"km","url":"https://samsunkesfet.com/muze/km/1","sections":30},
        {"id":2,"title":"Onur Anıtı","desc":"Samsun'un bağımsızlık mücadelesinin simgesi anıt.","cat":"Anıtlar","img":"https://samsunkesfet.com/media/img/muze/muzekapak/onur-aniti.jpg","lat":41.289066,"lon":36.336451,"hours":"7/24 Açık","audio":{"tr":"https://samsunkesfet.com/media/sound/tr/oa/onuraniti.mp3"},"code":"oa","url":"https://samsunkesfet.com/muze/oa/1","sections":1},
        {"id":3,"title":"Tütün İskelesi","desc":"Osmanlı dönemi tütün ticaretinin önemli merkezi.","cat":"Tarihi Mekanlar","img":"https://samsunkesfet.com/media/img/muze/muzekapak/tutun-iskelesi.jpg","lat":41.293000,"lon":36.340500,"hours":"09:00 - 17:30","audio":{"tr":"https://samsunkesfet.com/media/sound/tr/ti/tutun2.mp3"},"code":"ti","url":"https://samsunkesfet.com/muze/ti/1","sections":1},
        {"id":4,"title":"Alaçam Mübadele Müzesi","desc":"Türk-Yunan nüfus mübadelesini anlatan müze.","cat":"Müzeler","img":"https://samsunkesfet.com/media/img/muze/muzekapak/0.jpg","lat":41.605560,"lon":35.596940,"hours":"09:00 - 17:30","audio":{"tr":"https://samsunkesfet.com/media/sound/tr/ac/3.mp3"},"code":"ac","url":"https://samsunkesfet.com/muze/ac/1","sections":11},
        {"id":5,"title":"Amazon Köyü","desc":"Antik Amazon savaşçılarının izlerini taşıyan tarihi alan.","cat":"Tarihi Mekanlar","img":"https://samsunkesfet.com/media/img/muze/muzekapak/amazon-koyu.png","lat":41.323840,"lon":36.325460,"hours":"09:00 - 17:30","audio":{"tr":"https://samsunkesfet.com/media/sound/tr/az/1-amazon-g.mp3"},"code":"az","url":"https://samsunkesfet.com/muze/az/1","sections":11},
        {"id":6,"title":"Samsun Müzesi","desc":"Bölgenin en kapsamlı arkeoloji ve etnografya müzesi.","cat":"Müzeler","img":"https://samsunkesfet.com/media/img/muze/muzekapak/samsun-muzesi.jpg","lat":41.289720,"lon":36.333890,"hours":"09:00 - 17:30","audio":{"tr":"https://samsunkesfet.com/media/sound/tr/sm/sm.mp3"},"code":"sm","url":"https://samsunkesfet.com/muze/sm/1","sections":1},
        {"id":7,"title":"Sadi Tekkesi","desc":"Osmanlı dönemi dervişlerinin yaşadığı tarihi tekke.","cat":"Tarihi Mekanlar","img":"https://samsunkesfet.com/media/img/muze/muzekapak/sadi-tekkesi.jpg","lat":41.288220,"lon":36.337440,"hours":"09:00 - 17:30","audio":{"tr":"https://samsunkesfet.com/media/sound/tr/st/saditekkesi.mp3"},"code":"st","url":"https://samsunkesfet.com/muze/st/1","sections":1},
        {"id":8,"title":"İlkadım Anıtı","desc":"Atatürk'ün Samsun'a çıkışını simgeleyen anıt.","cat":"Anıtlar","img":"https://samsunkesfet.com/media/img/muze/muzekapak/ilkadim-aniti.jpg","lat":41.292336,"lon":36.335197,"hours":"7/24 Açık","audio":{"tr":"https://samsunkesfet.com/media/sound/tr/ia/1.mp3"},"code":"ia","url":"https://samsunkesfet.com/muze/ia/1","sections":1},
        {"id":9,"title":"Gazi Müzesi","desc":"Atatürk'ün Samsun'da kaldığı tarihi konak-müze.","cat":"Müzeler","img":"https://samsunkesfet.com/media/img/muze/muzekapak/gazi-muzesi.jpg","lat":41.290277,"lon":36.332500,"hours":"09:00 - 17:30","audio":{"tr":"https://samsunkesfet.com/media/sound/tr/gz/gazimuzesi.mp3"},"code":"gz","url":"https://samsunkesfet.com/muze/gz/1","sections":1},
        {"id":10,"title":"Havza Atatürk Evi","desc":"Atatürk'ün Havza'da kaplıca tedavisi gördüğü ev.","cat":"Müzeler","img":"https://samsunkesfet.com/media/img/muze/muzekapak/havza-ataturk-evi.jpg","lat":40.970637,"lon":35.658778,"hours":"09:00 - 17:30","audio":{"tr":"https://samsunkesfet.com/media/sound/tr/hv/1.mp3"},"code":"hv","url":"https://samsunkesfet.com/muze/hv/1","sections":20},
        {"id":11,"title":"Bandırma Vapuru","desc":"Atatürk'ün Samsun'a çıktığı tarihi gemi-müze.","cat":"Müzeler","img":"https://samsunkesfet.com/media/img/muze/muzekapak/bandirma-gemisi.jpg","lat":41.273530,"lon":36.367453,"hours":"09:00 - 17:30","audio":{"tr":"https://samsunkesfet.com/media/sound/tr/bv/bandirma-1.mp3"},"code":"bv","url":"https://samsunkesfet.com/muze/bv/1","sections":31},
        {"id":12,"title":"Bafra Tütün Müzesi","desc":"Bafra'nın tütün üretim tarihini anlatan müze.","cat":"Müzeler","img":"https://samsunkesfet.com/media/img/muze/muzekapak/bafra.jpg","lat":41.563740,"lon":35.898248,"hours":"09:00 - 17:30","audio":{"tr":"https://samsunkesfet.com/media/sound/tr/bf/101.mp3"},"code":"bf","url":"https://samsunkesfet.com/muze/bf/1","sections":18},
    ]
    ...
    async def proxy_samair_araclar():
        ...
        samair_line_codes = [
            ...
            'H3 BAFRA (YAKAKENT) - HAVALİMANI', 'H3 HAVALİMANI - BAFRA (YAKAKENT)',
            'H4 ÇARŞAMBA (TERME) - HAVALİMANI', 'H4 HAVALİMANI - ÇARŞAMBA (TERME)',
            'H5 HAVZA - HAVALİMANI', 'H5 HAVALİMANI - HAVZA'
        ]
        
        async def fetch_realtime(line_code):
            try:
                res = await asyncio.wait_for(
                    asyncio.to_thread(col.http.asis, 'RealTimeData', lineCode=line_code),
                    timeout=5
                )
                return res or []
            except Exception as e:
                log.warning(f"SamAir araç sorgusu hatası ({line_code}): {e}")
                return []

        tasks = [fetch_realtime(code) for code in samair_line_codes]
        ...
        for r in results:
            if isinstance(r, list):
                ...
                all_vehicles.extend(r.get('data', r.get('result', [])))
        ...
        for item in all_vehicles:
            ...
            raw_line = str(item.get('HatKodu', item.get('hatKodu', item.get('lineCode', 'SAMAIR'))))
            line_code_short = 'SAMAIR'
            m = re.match(r'^(H\d)', raw_line)
            if m:
                line_code_short = m.group(1)
            else:
                raw_name = str(item.get('HatAdi', item.get('lineName', '')))
                m2 = re.match(r'^(H\d)', raw_name)
                if m2:
                    line_code_short = m2.group(1)
    ...
    async def proxy_odak_araclar(hatid: int):
        ...
        try:
            ...
            if not rows:
                log.warning(f"Odak Araçlar: hatid={hatid} için veritabanında odak kaydı bulunamadı.")
                return JSONResponse({"active": True, "vehicles": []})
            
            odak_kodu = rows[0].get('kod', '').strip()
            if not odak_kodu:
                log.warning(f"Odak Araçlar: hatid={hatid} için odak kodu boş.")
                return JSONResponse({"active": True, "vehicles": []})
            
            # 2. ASIS API'sinden tüm hatları al
            orj_lines = await asyncio.to_thread(col.http.asis, "OrjLines")
            if not orj_lines or not isinstance(orj_lines, list):
                log.warning("Odak Araçlar: ASIS OrjLines listesi boş veya alınamadı.")
                return JSONResponse({"active": True, "vehicles": []})
            
            # 3. Odak kodunu (örn. G1) içeren ASIS lineCode'ları filtrele
            import re
            matched_line_codes = []
            pattern = re.compile(rf"\b{re.escape(odak_kodu)}\b", re.IGNORECASE)
            
            for item in orj_lines:
                line_code = item.get('lineCode', '')
                line_name = item.get('lineName', '')
                if pattern.search(line_code) or pattern.search(line_name):
                    matched_line_codes.append(line_code)
            
            matched_line_codes = list(set(matched_line_codes))
            if not matched_line_codes:
                log.warning(f"Odak Araçlar: '{odak_kodu}' kodu için eşleşen ASIS hattı bulunamadı.")
                return JSONResponse({"active": True, "vehicles": []})
            
            # 4. Eşleşen hatlar için paralel RealTimeData sorgusu yap
            async def fetch_realtime(lc):
                try:
                    res = await asyncio.wait_for(
                        asyncio.to_thread(col.http.asis, 'RealTimeData', lineCode=lc),
                        timeout=8
                    )
                    return res or []
                except Exception as e:
                    log.warning(f"Odak araç sorgu hatası ({lc}): {e}")
                    return []
            
            tasks = [fetch_realtime(lc) for lc in matched_line_codes]
            results = await asyncio.gather(*tasks)
            
            # 5. Sonuçları topla ve normalize et
            all_vehicles = []
            for r in results:
                if isinstance(r, list):
                    all_vehicles.extend(r)
    ...
    async def app_version(request: Request):
        ...
        if request.headers.get("sec-fetch-dest") == "document" or request.headers.get("accept", "").startswith("text/html"):
            ...
            return HTMLResponse("403 Forbidden - Direct access not allowed.", status_code=403)
    ...
    async def debug_proxy():
        ...
        # YBS testi
        try:
            # Token gerekmiyor, direkt sorgu yapıyoruz
            test_res = await asyncio.to_thread(
                http.ybs, 'odakSamsun_Crud', 'HatlarList', referer='https://odak.samsun.bel.tr/'
            )
            results["ybs_status"] = "OK" if test_res else "EMPTY_OR_FAILED"
            results["ybs_hat_count"] = len(test_res) if isinstance(test_res, list) else "?"
        except Exception as e:
            results["ybs_status"] = f"ERROR: {e}"
    ...
    async def admin_verify(key: str = ''):
        expected = ADMIN_KEY if ADMIN_KEY else "1919"
        if key == expected:
...
loadStats(); setInterval(loadStats, 10000);
    ...
    async def api_fiyat(code: str):
        ...
        hat = db.one("SELECT kat, name FROM hat WHERE code=?", (c,))
        if not hat:
            ilk = c.split()[0] if c.split() else c
            hat = db.one("SELECT kat, name FROM hat WHERE code LIKE ? OR name LIKE ?", (f'{ilk}%', f'%{c}%'))
            
        kat = hat['kat'] if hat else None
        ...
        # Kategori bazlı fiyat override ve tanımları
        if kat == 'ring':
            tam = 22.0
            indirimli = 16.0
            extra = [
                "Ring Hat Tarifesi: Tam 22.00 TL | Eğitim 16.00 TL",
            ...
            indirimli = 20.0
            extra = [
                "Hafif Raylı Sistem Kademeli Ücret Tarifesi",
                "1-24 İstasyon: Tam 26.00 TL | Eğitim 18.00 TL",
                "1-42 İstasyon: Tam 34.00 TL | Eğitim 20.00 TL",
                "Kampüs İçi: Tam 5.00 TL | Eğitim 3.00 TL",
                "1 Saat İçi Aktarma: Ücretsiz (Düşük hattan yüksek hatta geçişte fark alınır)",
                "Tramvay binişlerinde karttan maksimum ücret (34.00 TL) bloke edilir, inişte iade makinesinden iade alınır."
            ]
        elif kat == 'teleferik':
            tam = 50.0
            indirimli = 30.0
            extra = [
                "Teleferik Tarifesi: Tek Yön Tam 50.00 TL | Eğitim 30.00 TL",
                "Amisos Tepesi Teleferik hattında aktarma geçerli değildir."
            ]
        elif kat == 'havalimani':
            tam = 70.0
            indirimli = 70.0
            extra = [
                "Havalimanı Sefer Tarifesi: Sabit 70.00 TL",
                "Havalimanı hatlarında aktarma veya ücretsiz seyahat kartları geçerli değildir."
            ]
        elif kat == 'tekne':
            tam = 40.0
            indirimli = 25.0
            extra = [
                "Samsunum-1 / Atatürk Parkı Tekne Turu Tarifesi",
                "Tam: 40.00 TL | Öğrenci/Eğitim: 25.00 TL"
            ]
        else: # otobus, ekspres, ilce, odak vb.
            tam = 30.0
            indirimli = 20.0
            extra = [
                "Kent İçi Otobüs & Ekspres Tarifesi: Tam 30.00 TL | Eğitim 20.00 TL",
                "1 Saat İçi Aktarma: Ücretsiz | 1 Saat Sonrası: 8,00 TL",
                "Öğrenci Abonman: 50 Biniş 500 TL | Sınırsız 550 TL",
                "Sivil Abonman: 50 Biniş 1000 TL | Sınırsız 1100 TL"
            ]
            
        # Eğer DB'den gelen spesifik bir fiyat varsa ve varsayılan değilse koru
        if res.get("tam_fiyat"):
            tam = res["tam_fiyat"]
        if res.get("ogrenci_fiyat") or res.get("indirimli_fiyat"):
            indirimli = res.get("ogrenci_fiyat") or res.get("indirimli_fiyat")
